...
const postFilterDriver = async ({ searchField, searchValue, body, page }) => {
	let newBody;
	...
	try {
		const response = await webSiteAxios.post(
			routes.FilterDriver +
				`?page=${page}&searchValue=${searchValue}&searchField=${searchField}`,
			newBody
		);
...
export default postFilterDriver;
