...
const filterInvoiceForAllUser = async ({ searchField, searchValue, body }) => {
	let newBody;
	...
	try {
		const response = await webSiteAxios.post(
			routes.invoiceForAllUser + `?searchField=${searchField}&searchValue=${searchValue}`,
			newBody
		);
...
export default filterInvoiceForAllUser;
